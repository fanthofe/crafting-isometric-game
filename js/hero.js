...
function drawHero(g, px, py, dir, frame, walking, swing){
  ...
  // silhouette fine façon Maori : peau hâlée, chignon (tikitiki), tatouages tā moko
  ...
  P(3, 11+bob+ (legA<0?-1:0), 1, 3-(legA<0?-1:0), skin);     // jambes fines
  P(6, 11+bob+ (legA>0?-1:0), 1, 3-(legA>0?-1:0), skin);
  if(equip.pieds){ P(3, 12+bob, 1, 2, "#3e2d20"); P(6, 12+bob, 1, 2, "#3e2d20"); }  // bottes
  else { P(3, 13+bob, 1, 1, skinD); P(6, 13+bob, 1, 1, skinD); }                    // pieds nus

  P(2, 6+bob, 6, 4, torseC);                                  // torse nu ou tunique
  if(hasTorse){ P(2, 8+bob, 6, 1, "#6f4c30"); P(4, 6+bob, 2, 1, "#6f4c30"); }       // couture + col
  else { P(3, 7+bob, 1, 1, moko); P(5, 8+bob, 1, 1, moko); P(6, 7+bob, 1, 1, moko); } // tā moko de poitrine
  // pagne court, toujours porté
  P(2, 10+bob, 6, 2, "#a3754d");
  P(2, 10+bob, 6, 1, "#6f4c30");                              // ceinture
  ...
  if(!swingArm){
    ...
    if(!hasTorse){ P(1, 7+bob+arm, 1, 1, moko); P(8, 7+bob-arm, 1, 1, moko); }      // brassards tatoués
  } else {
  ...
  P(3, 1+bob, 5, 5, skin);                         // tête plus fine
  ...
  if(dir==="down"){ P(4,3+bob,1,1,"#1d1d22"); P(6,3+bob,1,1,"#1d1d22"); P(5,4+bob,1,1,moko); } // moko au menton
  ...
  } else {          // chignon traditionnel (tikitiki)
    P(4, 0+bob, 3, 1, hair);
    P(5, -1+bob, 2, 1, hair);
    P(5, -2+bob, 1, 1, hair);
  }
